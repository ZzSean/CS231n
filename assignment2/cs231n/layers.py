...
def batchnorm_backward(dout, cache):
    ...
    dsample_var = -0.5 * np.sum(dx_normalized * x_mu,
                                axis=0, keepdims=True) * sample_std_inv**3
    dsample_mean = -1.0 * np.sum(dx_normalized * sample_std_inv, axis=0,
                                 keepdims=True) - 2.0 * dsample_var * np.mean(x_mu, axis=0, keepdims=True)
...
def batchnorm_backward_alt(dout, cache):
    ...
    dsample_var = -0.5 * np.sum(dx_normalized * x_mu,
                                axis=0, keepdims=True) * sample_std_inv**3
    dsample_mean = -1.0 * np.sum(dx_normalized *
                                 sample_std_inv, axis=0, keepdims=True)
    dx1 = dx_normalized * sample_std_inv
    dx2 = 2.0 / N * dsample_var * x_mu
    dx = dx1 + dx2 + 1.0 / N * dsample_mean
    dgamma = np.sum(dout * x_normalized, axis=0, keepdims=True)
...
def conv_backward_naive(dout, cache):
    ...
    dx_padded = np.pad(
        dx, ((0, 0), (0, 0), (pad, pad), (pad, pad)), 'constant')
        ...
        for f in range(F):   # fth filter
            for j in range(H_new):
                for k in range(W_new):
                    ...
                    dx_padded[i, :, j * s:HH + j * s, k * s:WW + k *
                              s] += w[f] * dout[i, f, j, k]  # 上面的式子，关键就在于+号
    ...
    for i in range(N):
        for c in range(C):
            for j in range(H_new):
                for k in range(W_new):
                    out[i, c, j, k] = np.max(
                        x[i, c, j * stride:j * stride + pool_height, k * stride:k * stride + pool_height])
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(N):
        for c in range(C):
            for j in range(H_new):
                for k in range(W_new):
                    window = x[i, c, j * stride:j * stride +
                               pool_height, k * stride:k * stride + pool_height]
                    m = np.max(window)
                    dx[i, c, j * stride:j * stride + pool_height, k * stride:k *
                        stride + pool_height] = (window == m) * dout[i, c, j, k]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
