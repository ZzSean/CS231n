...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        #(2)
        captions_in_emb, emb_in_cache = word_embedding_forward(
            captions_in, W_embed)
        #(3)
        ...
        #backward and loss
        dtemp, grads['W_vocab'], grads[
            'b_vocab'] = temporal_affine_backward(dout, temporal_cache)
        if self.cell_type == 'rnn':
            dx, dh0, grads['Wx'], grads['Wh'], grads[
                'b'] = rnn_backward(dtemp, rnn_cache)
        elif self.cell_type == 'lstm':
            dx, dh0, grads['Wx'], grads['Wh'], grads[
                'b'] = lstm_backward(dtemp, lstm_cache)
        grads['W_embed'] = word_embedding_backward(dx, emb_in_cache)
        dfeatures, grads['W_proj'], grads[
            'b_proj'] = affine_backward(dh0, features_cache)
        #######################################################################
        #                             END OF YOUR CODE                             #
        #######################################################################
